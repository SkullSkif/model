...
// Генерация массива n (содержит числа от 1 до n)
vector<int> generateNArray(int size) {
    vector<int> arr(size);
    for (int i = 0; i < size; i++) {
        arr[i] = i + 1; // числа от 1 до n
    }
    return arr;
}

// Перемешивание массива n для случайного порядка
...
vector<vector<int>> samplingWithReplacement(const vector<int>& n, int m) {
    ...
    for (int i = 0; i < m; i++) {
        // Выбираем случайный индекс из массива n
...
vector<vector<int>> placement(const vector<int>& n) {
    ...
    // Перемешиваем индексы для случайного размещения
...
int main() {
    ...
    cout << "\nПояснения:\n";
    cout << "1. Массив n содержит числа от 1 до " << n_size << " в случайном порядке\n";
    cout << "2. Выборка с возвращением: элементы в выборке m могут повторяться\n";
    cout << "3. Выборка без возвращения: все элементы в выборке m уникальны\n";
    cout << "4. Размещение: частный случай выборки без возвращения при m = n\n";
    cout << "5. Многократные прогоны: суммирование результатов нескольких выборок\n";
    cout << "6. Матрица всегда размера m × n, где n - размер исходного массива\n";
